...
class ResNetBlock(tf.keras.Model):
    def __init__(self, dim):
        ...
        self.conv1 = tf.keras.layers.Conv2D(dim, (3, 3),
                                            padding='valid',
                                            use_bias=False)
        ...
        self.conv2 = tf.keras.layers.Conv2D(dim, (3, 3),
                                            padding='valid',
                                            use_bias=False)
...
def make_generator_model(n_blocks):
    ...
    # Decoding
    model.add(
        tf.keras.layers.Conv2DTranspose(128, (3, 3),
                                        strides=(2, 2),
                                        padding='same',
                                        use_bias=False))
    model.add(tf.keras.layers.BatchNormalization())
...
def make_discriminator_model():
    ...
    model.add(
        tf.keras.layers.Conv2D(128, (4, 4),
                               strides=(2, 2),
                               padding='same',
                               use_bias=False))
    model.add(tf.keras.layers.BatchNormalization())
    ...
    model.add(tf.keras.layers.Conv2D(1, (4, 4), strides=(1, 1),
                                     padding='same'))
